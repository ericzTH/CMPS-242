...
# | -------------                  |:-------:| :----:| :----: |:-----:|:-----:|
# | Training accuracy              | 97.37   | 97.33 | 97.16  | 95.33 | 80.70 |
# | validation accuracy            | **95.63**   | 95.53 | 95.16  | 93.60 | 78.9  |
# | Test accuracy (with l2 penalty)| **95.48**   |   -   |   -    |   -   |   -   |
# | Test accuracy (with l1 penalty)| **86.74**   |   -   |   -    |   -   |   -   |
# <br>
#     *Time taken for stochastic dradient descent :621.94 seconds* <br>
#     *Time taken for mini batch dradient descent with batch size = 50 :468.02 seconds *<br>
#     *Time taken for batch gradient descent = 833.349572 *<br>
...
# $$\eta = \eta_0.t^{-\alpha} $$
# where $\alpha =0.9$
# here I set $\eta_0 = 50$ and used more number of iterations so that the net learning rate is not too small.
# * Update Rule:
...
def bgd_optimizer(w,matrix,n_iters = 100,reg = 0.01, penalty = 'l2'):
	...
	for i in range(1,n_iters+1):
		if i%250==0:
			print("\t iteration %d of %d. Cost = %r"%(i,n_iters,costfn(w,matrix = matrix ,reg = reg,penalty = penalty)))
...
# 
# ** Note ** : Setting batch_size to 1 gives Stochastic Gradient Descent.
...
def kfold(matrix,w,n_splits = 10,n_iters = 100,reg = 0.01, penalty = 'l2'):
    ...
    for train_index,test_index in kf.split(matrix):
        ...
        fold+=1
        #print("Train indices length : %d, \t Test indices length: %d"%(len(train_index),len(test_index)))
...
mins = int(ttl/60)
sec = (ttl)%60
#print("Time taken to train the model with batch gradient descent and \'l2\' penalty for 1000 iterations:%d hrs %d mins %f sec :"%(hrs,mins,sec))
plt.plot(np.log(reg_list),train_acc_list,label = 'training')
plt.plot(np.log(reg_list),valid_acc_list,label = 'validation')
plt.legend()
...
for i in wts_fold_list.keys():
    ...
    print("accuracy with reg = %.5f : %.4f"%(i,accs))
...
print("Accuracy on test data with lambda : %f = %f"%(0.001,100*(metrics.accuracy_score(test[:,-1],yhat))))
...
print(train.shape)
start = time.time()
w_withl1 = bgd_optimizer(matrix = train,w = initial_w,n_iters = 1000, reg = 0.001,penalty = 'l1')
end = time.time()
print("Time taken for batch gradient descent = %f"%(end-start))
yhat_withl1 = predict(sigmoid(np.dot(test[:,:-1],w_withl1)))
print("Accuracy on test data with lambda : %f = %f"%(0.001,100*(metrics.accuracy_score(test[:,-1],yhat_withl1))))
...
start = time.time()
w = minibatch_optimizer(w = w, matrix = train,n_iters=1000, batch_size = 50, reg = 0.001)
end = time.time()
print("Time taken for mini batch dradient descent with batch size = 50 :%f"%(end-start))
yhat_mb = sigmoid(np.dot(test[:,:-1],w))
yhat_mb = predict(yhat_mb)
print("accuracy:%.4f"%(metrics.accuracy_score(test[:,-1],yhat_mb))) # always ytrue and ypred
...
initial_w = np.random.randn(train.shape[1]-1,1)
start = time.time()
w = minibatch_optimizer(w = w, matrix = train,n_iters=1000, batch_size = 1, reg = 0.001)
end = time.time()
print("Time taken for stochastic dradient descent:%f"%(end-start))
yhat_sgd = sigmoid(np.dot(test[:,:-1],w))
yhat_sgd = predict(yhat_mb)
print("accuracy:%.4f"%(metrics.accuracy_score(test[:,-1],yhat_sgd))) # always ytrue and ypred
...
def bgd_optimizer_bias(w,x,y,n_iters = 100,reg = 0.01, penalty = 'l2'):
	...
	for i in range(1,n_iters+1):
		#print(i)
		if i%250==0:
			print("\t iteration %d of %d. Cost = %r"%(i,n_iters,cost_bias(w,x,y,reg = reg)[0]))
			#learning_rate = 50*np.power(i,-0.9)  #eta = eta0*(iteration^-0.9)
		...
		#print(w.shape,delta.shape)
	return w
w_opt = bgd_optimizer_bias(w_bias,train_bias_x,y,reg = 0.001,n_iters = 1000)
yhat_bias = predict(sigmoid(np.dot(train_bias_x,w_opt).reshape((train_bias_x.shape[0],1))))

                    #yhat_bias = predict())
print(100*metrics.accuracy_score(y,yhat_bias))
