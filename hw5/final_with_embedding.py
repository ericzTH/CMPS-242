...
with tf.name_scope('Embedding_matrix') :
	embedding_matrix = tf.Variable(tf.random_normal([10,len(vocab_dict)]))
		...
		for k in range(10):
			...
			for y in range(len(text[k].split(' '))):
				if (text[k]).split(' ')[y] in vocab_dict :
					u += 1
			#matrix1 = []
			matrix1=np.zeros(shape=(10,u))
			for y in range(len(text[k].split(' '))):
				if (text[k]).split(' ')[y] in vocab_dict :
					#print("Word \""+str((text[k]).split(' ')[y])+"\" found in dict")
					b = h_iden[vocab_dict[(text[k]).split(' ')[y]]].tolist() 
					for i in b:
						if i == 1:
							print(b.index(i))
					#print(b[8854:8860])
					matrix1[:][y] = tf.transpose(tf.gather(tf.transpose(embedding_matrix)[:][b.index(1)]),b.index(1))
...
"""	for k in range(50):
		...
		for y in range(len(test_text[k].split(' '))):
			if (test_text[k]).split(' ')[y] in vocab_dict :
				#print("Word \""+str((text[k]).split(' ')[y])+"\" found in dict") 
				matrix_test.append(embedding_matrix[vocab_dict[(test_text[k].split(' '))[y]]]) # append ohv of each word in the current tweet
